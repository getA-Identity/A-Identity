...
<Frame caption="The three layers side by side, with the module each one lives in. Layer 2 is the source of truth because the USDC sits inside the contract, so the cap is enforced by code rather than by a server that could be bypassed. Layers 1 and 3 are a pre-check and a hosted screen around it.">
  <img src="/images/architecture-enforcement.png" alt="Three cards: server pre-check labelled advisory, on-chain policy vault labelled source of truth, and Circle Agent Wallet labelled hosted screening" />
</Frame>
...
<Steps>
  <Step title="Server policy engine (pre-check)">
    ...
    source of truth: it is the fallback, not the guarantee.
  </Step>
...
At settlement, `executeInstruction` routes a payment **vault, then Circle, then direct**, each
...
policy). They are genuinely separate keys, and the agent can never change its own limits.
</Note>
...
| On-chain vault | The same policy, on Arc, where an over-limit `pay()` reverts | Trustless source of truth |
