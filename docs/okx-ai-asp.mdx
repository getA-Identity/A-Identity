...
<CardGroup cols={2}>
  ...
  <Card title="counterparty_check" icon="scale-balanced">
    **$0.008** &middot; deal-specific verdict between two agents + same-operator self-deal check, plus a same-human check against the Self Agent ID registry on Celo.
  </Card>
...
| `GET /stats` | live on-chain settlement total (payTo's current USD₮0 balance) |
