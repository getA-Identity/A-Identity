...
description: "Robinhood Chain network config for both networks, the canonical ERC-8004 registries already deployed there, and the first x402 facilitator the chain has."
---
...
An x402 facilitator is the service a seller calls to verify a payment and settle it. This
chain had none from anyone, which meant a seller here could not accept a gasless x402
payment at all. We run one, and it is open:
