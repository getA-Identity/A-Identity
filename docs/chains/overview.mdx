...
| Algorand | `algorand:wGHE2Pwdvd7S12BL5FaOP20EGYesN73k` | beta | Native Circle USDC (ASA 31566704), x402 v2 through the GoPlausible facilitator: the buyer signs a fee-zero transfer and the atomic group's pooled fee covers it. Credential-gated until a funded, opted-in payTo exists; no ERC-8004 registry found on Algorand, so identity stays bridged |
