...
| Stellar | `stellar:pubnet` | live | An `AgentSpendPolicy` Soroban vault holding real Circle USDC under a 1 USDC daily cap; real money has moved under the on-ledger policy, including a typed `DailyCapExceeded` refusal. The x402 rail is configured for `stellar:testnet`, so no paid call sells on pubnet yet. No ERC-8004 on either Stellar network: that standard is EVM-only |
