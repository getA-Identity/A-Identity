...
fn the_ttl_constants_are_the_numbers_the_safety_argument_rests_on() {
    ...
    // The relations these lines used to assert at runtime now live in storage.rs as
    // `const _: () = assert!(...)`, so clippy correctly reports a runtime copy as having a
    // constant value. That is the fix working rather than a lint to silence: the same
    // conditions are checked by rustc, so a violating edit stops the BUILD instead of
    // failing this test. The absolute values above are still worth asserting here, because
    // a const assertion proves the relation and not the number.
}
