...
fn withdraw_is_reachable_in_every_reachable_state() {
    /// A named state the contract can be driven into, and the call that gets it there.
    type State = (&'static str, fn(&Setup));

    let cases: [State; 7] = [
        ("frozen", |s| {
            s.client().set_frozen(&true);
        }),
        ("allowlist on with an empty list", |s| {
        ...
        ("zero cap and zero ceiling", |s| {
            s.client().set_policy(&0, &0, &false);
        }),
        ("operator rotated to a stranger", |s| {
            s.client().set_operator(&s.stranger);
        }),
        ("instance idled far past its TTL floor", |s| {
            s.advance_ledgers(200 * 17_280);
        }),
    ];
    ...
    for (name, apply) in cases {
        ...
        assert!(
            held > 0,
            "{name}: the probe needs a funded vault to mean anything"
        );
        s.client().withdraw(&s.owner, &held);
        assert_eq!(
            s.client().balance(),
            0,
            "{name}: withdraw could not empty the vault"
        );
    }
}
...
fn a_refused_payment_writes_nothing() {
    ...
    assert_eq!(
        ...
        spent_before,
        "a refusal moved the day accumulator"
    );
    assert_eq!(
        s.client().balance(),
        balance_before,
        "a refusal moved the vault balance"
    );
}
...
fn worst_case_pay_fits_the_pubnet_budget() {
    ...
    assert!(
        ...
        "worst-case pay exceeds the pubnet CPU limit"
    );
    assert!(
        mem < TX_MEMORY_LIMIT,
        "worst-case pay exceeds the pubnet memory limit"
    );
    ...
    assert!(
        cpu < TX_MAX_INSTRUCTIONS / 10,
        "worst-case pay CPU regressed past 10% of the pubnet limit"
    );
    assert!(
        mem < TX_MEMORY_LIMIT / 10,
        "worst-case pay memory regressed past 10% of the pubnet limit"
    );
}
