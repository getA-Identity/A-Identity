...
export const PROVENANCE: ChainProvenance[] = [
  {
    chain: 'arc-mainnet',
    summary:
      'Arc Mainnet opened publicly on 2026-09-16, and the same day agent #0 on its canonical ERC-8004 registry became ours: the first token that registry minted. Paid trust calls settled here on both of our rails that day, our own EIP-3009 facilitator and Circle Gateway nanopayments, an AgentSpendPolicy vault holds real USDC under dust caps with its source verified on Sourcify, and our oracle validator anchored the agent\'s reputation. Since 2026-09-25 the vault\'s agent key is a different key from its owner\'s, and settlements have also gone through Arc\'s predeployed Memo and Multicall3From contracts. Gas on this chain is USDC itself, so every cost below is an exact dollar figure with no price feed. The operating wallet was funded by burning native Circle USDC on Base through CCTP V2 with Circle\'s Forwarding Service, which minted it here without any Arc gas of ours.',
    agent: { tokenId: '0', caip: 'eip155:5042:8004/0', owner: OWNER, tokenUri: AGENT_CARD },
    contracts: [
      {
        name: 'IdentityRegistry',
        address: '0x8004a169fb4a3325136eb29fa0ceb6d2e539a432',
        note: 'The canonical mainnet ERC-8004 registry, deployed by its authors, at the same address Base, Arbitrum One, Robinhood Chain, X Layer and Celo carry. Verified 2026-09-16 by reading the EIP-1967 implementation slot here and on Base and matching the implementation code, because a 130-byte proxy at an address proves nothing on its own. Arc testnet\'s 0x8004A818 registry set has no code on this chain.',
      },
      { name: 'ReputationRegistry', address: '0x8004BAa17C55a88189AE136b182e5fdA19dE9b63' },
      {
        name: 'USDC (ERC-20 interface over native gas USDC)',
        address: '0x3600000000000000000000000000000000000000',
        note: 'The settlement token and the gas token are one balance: 18 decimals natively, 6 through this interface. EIP-3009 confirmed by authorizationState, and the EIP-712 domain (name "USDC", version "2") reproduces the live DOMAIN_SEPARATOR with chainId 5042.',
      },
      {
      ...
      },
      {
        ...
        note: 'Circle\'s, not ours. Our buyer wallet deposited into it so the Gateway rail could be exercised.',
      },
      {
        name: 'Memo',
        address: '0x5294E9927c3306DcBaDb03fe70b92e01cCede505',
        note: 'Arc\'s predeployed transaction-memo contract, not ours. It forwards the inner call with the original sender preserved and emits a Memo event indexed by memoId, which is how a payment carries its reason on chain.',
      },
      {
        name: 'Multicall3From',
        address: '0x522fAf9A91c41c443c66765030741e4AaCe147D0',
        note: 'Arc\'s predeployed batching contract, not ours. Unlike Multicall3 it keeps the caller as msg.sender for every inner call, so a batch of USDC transfers still comes from our own address.',
      },
    ],
    artifacts: [
      {
        kind: 'funding',
        label: 'Buyer USDC on Base swept into the operator wallet (EIP-3009, buyer signed, operator broadcast)',
        txHash: '0x2ffd849f608224c0e9d424e996e522b0f89130c7573df1f0ff9cc565ce423a9d',
        onChain: 'base',
        blockNumber: 51385662,
        note: '1 USDC from the buyer wallet, which holds no ETH and so cannot broadcast, into the operator wallet that burns. 85752 gas.',
      },
      {
        kind: 'bridge',
        label: 'CCTP V2 burn on Base with the cctp-forward hook: 1.012 USDC to Arc Mainnet',
        txHash: '0x6c376f51dbea8e66d76fcacba1b083a78a692105e130cc95fc719fd3e050fddb',
        onChain: 'base',
        blockNumber: 51385811,
        note: 'depositForBurnWithHook to domain 26, fast finality, maxFee 0.066903 USDC covering the forwarding fee and the protocol fee. Approval tx 0x6e7a653c. Driven by mcp/scripts/cctp-forward-bridge.mjs.',
      },
      {
        kind: 'bridge',
        label: 'The forwarded mint on Arc Mainnet: 0.945097 USDC arrived, broadcast by Circle',
        txHash: '0x1f3dd9785af195765f3fa57dfd9f4f32142c8a206c22537594c51f9dcdc55414',
        onChain: 'arc-mainnet',
        blockNumber: 21155781,
        note: 'Sent by Circle\'s forwarder (0x8df10e31...), not by us, 16 s after the burn receipt, and its gas came out of the burned amount. Counted as arrived only after the receipt succeeded and our own balance read moved from 0 to 0.945097.',
      },
      {
        kind: 'mint',
        label: 'Agent #0 minted through the production adapter',
        txHash: '0x1d9f57113cee71e6a0aa73d0905d02f88a08877489c05828e604db2a48412844',
        onChain: 'arc-mainnet',
        blockNumber: 21156191,
        note: '194916 gas at 143.09 gwei, 0.0279 USDC. ownerOf(0) reads back to the operator wallet and tokenURI(0) to the agent card.',
      },
      {
        kind: 'funding',
        label: 'Oracle validator given gas for its attestation',
        txHash: '0x6657fbd26e83d6ff88299fc148a55fbb307ed746c025d0f0318684efd725501b',
        onChain: 'arc-mainnet',
        blockNumber: 21156339,
        note: 'A native USDC transfer sized from the live gas price (750k gas worth), because a fixed ether-sized top-up would be a thousandth of one write here.',
      },
      {
        kind: 'attestation',
        label: 'Agent #0\'s reputation anchored on the canonical ReputationRegistry',
        txHash: '0xbc70c4ce2ded14e17809d6cdcb2d365f85b4fe22e77957890bf1dac3bf1ecee9',
        onChain: 'arc-mainnet',
        blockNumber: 21156348,
        note: 'giveFeedback from our oracle validator (0xee602A16..., the identity that attests on Arc testnet, Robinhood Chain, Arbitrum One and Base). Score 60 of 1000, tagged onchain-identity-basis. 301783 gas, 0.0389 USDC. Deliberately NOT added to asp/attestations.ts: that list is matched by token id alone, so a second #0 would change what the registered OKX.AI tools return for Robinhood Chain\'s #0.',
      },
      {
        kind: 'deploy',
        label: 'An AgentSpendPolicy vault, live on Arc Mainnet with real USDC under dust caps',
        txHash: '0xabf41b6f1245f0c265dbe9b3e60488b4776b54e7a4ee263b18b76dd35268e653',
        onChain: 'arc-mainnet',
        blockNumber: 21156384,
        note: '930785 gas, 0.1191 USDC. Deployed with the operating wallet as both owner and operator, which the split below undoes. The first session key was set to expire 2026-09-23 (tx 0x6614754b), and the vault was funded with 0.03 USDC (tx 0x7293cf6a).',
      },
      {
        kind: 'settlement',
        label: 'The vault paid 0.01 USDC inside its policy, and refused a 0.50 payment by contract',
        txHash: '0x9975968a2c23096269724b66f483229366500c2214297a3a6b5b2059a472dc05',
        onChain: 'arc-mainnet',
        blockNumber: 21156403,
        note: 'The in-policy payment is this transaction. The refusal has no hash by design: a payment above autoApproveMax was simulated and came back as the contract\'s own typed AboveAutoApprove error, and nothing was broadcast. Vault state after: balance 0.02, spentToday 0.01.',
      },
      {
        kind: 'funding',
        label: 'Buyer wallet funded on Arc Mainnet from the operator wallet',
        txHash: '0x04cbbabe1dd82826e30778398677f1afcd9cdea956fd34992aeaad5ee0b226d5',
        onChain: 'arc-mainnet',
        blockNumber: 21156564,
        note: '0.3 USDC, read back on the buyer before either sale was attempted.',
      },
      {
        kind: 'funding',
        label: 'Buyer deposited 0.05 USDC into Circle Gateway',
        txHash: '0x894781bd2dad1de2f9ab400c13ba95589e9b88285ea369b4172c8162cdfb133a',
        onChain: 'arc-mainnet',
        blockNumber: 21156917,
        note: 'Approval tx 0xd85b9519. Counted only once Gateway\'s own /v1/balances showed 0.05 available on domain 26. This balance paid the Gateway sale below.',
      },
      {
        kind: 'settlement',
        label: 'First Circle Gateway nanopayment, settled on-chain in Gateway\'s batch (verify_agent, 0.001 USDC)',
        txHash: '0x8a47eac41acdc0b84de73b4e83ffb5bef72d86e5c8d2f4bf6574a646bc04c6e7',
        onChain: 'arc-mainnet',
        blockNumber: 21158238,
        note: 'The buyer signed against GatewayWalletBatched from its Gateway balance and paid no gas; neither did we. Served only after Gateway\'s transfers API returned the transfer for the authorization nonce, to our payTo, for the price (transfer 5958b158, credited 2026-09-16T12:26Z), and marked completed when Gateway reported this batch, sent by Circle to the GatewayWallet.',
      },
      {
        kind: 'settlement',
        label: 'First x402 settlement through our EIP-3009 facilitator (verify_agent, 0.021 USDC)',
        txHash: '0xd44287c8d73790b818ab73016dc21483260d1948d58ac95e8a169be1d750df9c',
        onChain: 'arc-mainnet',
        blockNumber: 21157204,
        note: 'The buyer signed and paid no gas; we broadcast. 87165 gas at 169.75 gwei, exactly 0.0148 USDC, and the receipt carries the matching 21000-unit USDC Transfer from the buyer to the receiving address. This measurement set the disclosed fee (0.03). The first attempt was refused before broadcast by our own gas ceiling, which was denominated in ether; the ceiling is now per gas unit. Served by a local instance of the facilitator before the production service held the Arc key, so it is not in the production log, which starts with the sale below.',
      },
      {
        kind: 'settlement',
        label: 'Circle Gateway sale served by the production backend (verify_agent, 0.001 USDC)',
        txHash: '0xd2006085f741e91deacfdeb82ee622a31200e8f959f5807d9a5dfe0d45f71de9',
        onChain: 'arc-mainnet',
        blockNumber: 21163923,
        note: 'The first sale on the deployed service rather than a local run: credited by Gateway at 2026-09-16T13:09Z and applied in this batch, which Circle sent. The payer is our buyer wallet, so the production ledger counts it as internal.',
      },
      {
        kind: 'settlement',
        label: 'EIP-3009 sale served by the production backend (verify_agent, 0.031 USDC)',
        txHash: '0xa3e67cbd50b941aa794ea4fe1428a06c3a23c100fb0bf9753d8f4bcc20143f93',
        onChain: 'arc-mainnet',
        blockNumber: 21187928,
        note: 'Broadcast by the deployed service with ARC_MAINNET_SIGNER_KEY. 87109 gas at 20.03 gwei, 0.001745 USDC, against the 0.03 fee set on launch day at 169.75 gwei. Buyer and receiver are both ours, labeled internal.',
      },
      {
        kind: 'session-key',
        label: 'Vault session key renewed to 2026-10-24 by its owner',
        txHash: '0xc6c0449e230fb95607b6e823686b5d068e4277be1c8edd75a7da4fe693666c82',
        onChain: 'arc-mainnet',
        blockNumber: 22525966,
        note: 'The first key lapsed on 2026-09-23 and pay() reverted SessionKeyExpired from then on while every other gate stayed as set. An owner-only setSessionKeyExpiry through mcp/scripts/evm-vault-session.mjs, 29713 gas, 0.000594 USDC; an operator pay() simulated as allowed afterwards.',
      },
      {
        kind: 'settlement',
        label: 'Two Circle Gateway sales served by the production backend, settled in one batch (verify_agent, 0.001 USDC each)',
        txHash: '0x1338762a8203452d75dfeea76a33fa296c299c6e9fc297bec9112bbe801e0413',
        onChain: 'arc-mainnet',
        blockNumber: 22547671,
        note: 'Credited by Gateway at 2026-09-24T16:42Z and 16:43Z (transfers c44c0e6f and e06f2175) and applied together in this batch, which Circle sent. Internal, like every payment on this chain so far.',
      },
      {
        kind: 'session-key',
        label: 'The vault\'s agent key split from its owner key (setOperator)',
        txHash: '0xe469905f8836dc4fa3262bf8f8420870af3bc495fe72ec143160485e502408a4',
        onChain: 'arc-mainnet',
        blockNumber: 22711580,
        note: 'Owner-only setOperator to the buyer agent 0x8C8D...3349 through mcp/scripts/evm-vault-operator.mjs, 30123 gas, 0.000605 USDC. Simulated right after, nothing broadcast: the new operator is refused withdraw, setPolicy, setSessionKeyExpiry, ownerPay and setOperator with NotOwner and anything above 0.25 USDC with AboveAutoApprove, and the owner is refused pay() with NotOperator. Before this, one key held both roles and the caps bound nobody who held it.',
      },
      {
        kind: 'settlement',
        label: 'The agent key paid 0.01 USDC from the vault inside its policy',
        txHash: '0xc1f3eb7fd7e8a7eccb2d36e309ea8f54c6e188d0687bbf1686a3d5e0165c7ae4',
        onChain: 'arc-mainnet',
        blockNumber: 22711585,
        note: 'pay() sent by the operator 0x8C8D...3349, not by the owner: Paid(byOwner=false), and the vault balance read 0.02 before and 0.01 after. 90341 gas, 0.001816 USDC. The payee is the owner, so the money stayed with us.',
      },
      {
        kind: 'settlement',
        label: 'A USDC payment carrying its reason on chain through Arc\'s Memo contract',
        txHash: '0xbd24e32c63b0a2451a7ba65c4cdaab45ad221213ac1af75f954dea73faa300c6',
        onChain: 'arc-mainnet',
        blockNumber: 22711669,
        note: '0.01 USDC sent to our own wallet through Memo.memo(), so only gas moved (63108 gas, 0.001268 USDC). The Memo event reads back by memoId 0xb511f691 with the reason {agent eip155:5042:8004/0, instruction, service payment, decision auto_approved}, the sender preserved as our wallet and the target the USDC contract. Driven by mcp/scripts/test-memo.mjs --chain arc-mainnet, 8 of 8 checks.',
      },
      {
        kind: 'settlement',
        label: 'Three USDC transfers settled atomically in one transaction through Multicall3From',
        txHash: '0xc12e2dcbda2c4207dc5a8305f0a2dd57b1aa581dc963587dcd1f64bb757eb5bf',
        onChain: 'arc-mainnet',
    ...
    caveats: [
      'Every payment recorded here is self-funded: every payer, every receiving address and every account that broadcasts is ours, except Circle\'s Gateway batches and forwarded mint. It is labeled internal and always will be. This is evidence the rails work on Arc Mainnet, not evidence of demand.',
      'The Circle Gateway sale is Circle\'s batch transaction, not ours: we broadcast nothing on that rail. Gateway credited the call first (transfer 5958b158-6603-4165-a0a9-cfab2bec2ea7) and applied the net position on-chain later in a batch that may carry other parties\' transfers too, so the batch hash proves settlement of a net position rather than a one-to-one payment.',
      'We deployed nothing here except the vault. The registries, USDC, CCTP, Gateway, Memo and Multicall3From are their authors\' contracts; registration is permissionless.',
      'The vault\'s owner is still the operating wallet, a server-held key, and this contract has no owner transfer, so moving the owner to a wallet a person holds means deploying a new vault. What the 2026-09-25 split fixed is the other half: the agent key can no longer act as the owner.',
      'The Memo and Multicall3From receipts are test settlements to our own wallet. The product\'s payment path on this chain does not route through either contract yet, and the /api/arc/memos read still points at Arc testnet.',
      'The block explorer, explorer.arc.io, opens in a browser without signing in, but its JSON API answers non-browser clients with a bot challenge, and docs.arc.io still calls it permissioned. Every transaction here is also readable from any Arc Mainnet RPC.',
      'There is no ValidationRegistry in this family, so KYA cannot be anchored on Arc Mainnet. Arc\'s reference ERC-8183 escrow is documented for testnet only and has no code here, so escrow stays on testnet.',
      'Circle Paymaster has no contract on Arc Mainnet, and the account-abstraction, treasury, airdrop and Circle developer-wallet paths are still bound to Arc testnet. They move rail by rail, each on its own evidence.',
      'Gas moved from about 35 to about 187 gwei over launch day, and the fee (0.03 USDC) was set at 2x the settlement measured then. Since then gas has sat near the 20 gwei floor, where the same settlement costs about 0.0017 USDC, so the fee is conservative; it is re-measured if the effective gas price passes about 340 gwei.',
    ],
  },
  {
    chain: 'rhchain',
  ...
  },
  {
    ...
    contracts: [
      ...
      },
      {
        ...
        address: 'CC5RNXNHKKPAHFP5YEOTZDFOQDQVC6AQKX3EH3W6QKFKGVBAXPVM3RWA',
        note: 'The instance is ours, the code is not. OpenZeppelin\'s Stellar smart account, deployed through the `smart-account-kit` npm package 0.8.0 (stellar/smart-account-kit, formerly kalepail) against account wasm hash 1b5f4534..., with OpenZeppelin\'s testnet WebAuthn verifier CC7EKIHQ... as its one signer\'s verifier. Both of those are third-party testnet deployments read out of the kit\'s deployment manifest, not built or verified by us. The kit says of itself that it is unaudited and that the contracts under it were audited at rc-v0.7.0 with a different scope; we repeat that rather than improve on it.',
      },
      {
...
export const PROOF_RAILS: ProofRail[] = [
  {
    ...
    title: 'Circle Arc',
    lede:
      "Mainnet and testnet. On Arc Mainnet, the day it opened (2026-09-16), agent #0 on the canonical registry became ours, a spend vault took real USDC, and paid calls settled through both our own EIP-3009 facilitator and Circle Gateway nanopayments, all funded by a CCTP burn on Base that Circle minted here. On Arc testnet, the phase-1 chain, all three ERC-8004 registries run together: agent #849980 (Meridian) was KYA-attested through the ValidationRegistry and carries the first reputation anchor our oracle ever wrote.",
    chains: ['arc-mainnet', 'arc'],
  },
  {
    slug: 'stellar',
    title: 'Stellar',
